...
task("totalSupply", "Total supply of the ERC-20 token.")
    .setAction(async function (_, hre) {
      console.log(`ChainId: ${await (hre.getChainId())}`);
      const token = await hre.ethers.getContract("Coin");
      const supply = await token.totalSupply();
      const tokenSymbol = await token.symbol();
      console.log(`ERC-20 contract has a total supply of:  ${supply} ${tokenSymbol}`);
});
...
task("balanceOf", "Total balance of ERC-20 token for a given account.")
    ...
    .setAction(async function ({ account }, hre) {
      ...
      const tokenSymbol = await token.symbol();
      console.log(`Account ${account} has a total token balance:  ${balance} ${tokenSymbol}`);
});
